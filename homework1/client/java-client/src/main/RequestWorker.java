...
class RequestWorker implements Runnable {
    ...
    public RequestWorker(String serverUrl, CountDownLatch latch, int count, BlockingQueue<String> queue) {
    ...
    private String getLogEntry(Instant time, long start, long end, String type, int responseCode) {
        ...
        return start + "," + type + "," + latency + "," + responseCode;
    }
                ...
                for (int i = 0; i < MAX_RETRIES; i++) {
                    ...
                    try (OutputStream os = postConnection.getOutputStream()) {
                        ...
                        if (postResponseCode == HttpURLConnection.HTTP_OK) {
                            ...
                            String logPost = getLogEntry(postStartTimestamp, postStartTime, postHTTPTime, "POST", postResponseCode);
                            //System.out.println(logPost);
                            this.queue.put(logPost);

                            String jsonPostResponse = getHttpResponse(postConnection.getInputStream());
                            i = 100;
                            break;
                            // System.out.println("POST success. " + jsonPostResponse);
                        } else {
                ...
                for (int i = 0; i < MAX_RETRIES; i++) {
                    ...
                    try {
                        ...
                        if (getResponseCode == HttpURLConnection.HTTP_OK) {
                            ...
                            String logGet = getLogEntry(getStartTimestamp, getStartTime, getHTTPTime, "GET", getResponseCode);

                            //System.out.println(logGet);
                            this.queue.put(logGet);

                            // Read and process the GET response if needed
                            String jsonResponse = getHttpResponse(getConnection.getInputStream());
                            i = 100;
                            break;
                            // System.out.println("Get success. " + jsonResponse.toString());
                        } else {
